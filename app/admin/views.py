...
def change_filename(filename):
    fileinfo = os.path.splitext(filename)  # 切割成名字和后缀两部分
    # 文件名以时间+随机字符串+后缀 拼接一起
...
def logout():
    ...
    session.pop('admin_id', None)  # 删除sessionid
    return redirect(url_for("admin.login"))
...
def pwd():
    ...
    if form.validate_on_submit():
        data = form.data
        admin = Admin.query.filter_by(name=session['admin']).first()  # 查找账号
...
@admin_login_req
@admin_auth
def tag_del(id=None, page=None):  # 删除跳转到当前页
    tag = Tag.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
...
def tag_edit(id=None):
    form = TagForm()
    tag = Tag.query.get_or_404(id)  # 查找要编辑的标签
    if form.validate_on_submit():
        data = form.data
...
def movie_add():
    ...
    if form.validate_on_submit():
        data = form.data
        # secure_filename函数来操作安全文件名
        file_url = secure_filename(form.url.data.filename)
        file_logo = secure_filename(form.logo.data.filename)
        # 判断上传文件的文件夹是否存在
        if not os.path.exists(app.config["UP_DIR"]):
            os.makedirs(app.config["UP_DIR"])  # 如果不存在 就创建一个文件夹
            os.chmod(app.config["UP_DIR"], "rw")  # 给一个可读可写的权限
        # 修改文件名
...
# 删除电影
@admin.route('/movie/del/<int:id>/<int:page>', methods=['GET'])
@admin_login_req
@admin_auth
def movie_del(id=None, page=None):  # 删除跳转到当前页
    movie = Movie.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
...
def preview_add():
    ...
    if form.validate_on_submit():
        data = form.data
        file_logo = secure_filename(form.logo.data.filename)  # secure_filename函数来操作安全文件名
        # 判断上传文件的文件夹是否存在
        if not os.path.exists(app.config["UP_DIR"]):
            os.makedirs(app.config["UP_DIR"])  # 如果不存在 就创建一个文件夹
            os.chmod(app.config["UP_DIR"], "rw")  # 给一个可读可写的权限
        logo = change_filename(file_logo)  # 修改文件名
...
@admin_login_req
@admin_auth
def preview_del(id=None, page=None):  # 删除跳转到当前页
    preview = Preview.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
...
def preview_edit(id=None):
    form = PreviewForm()
    form.logo.validators = []
    preview = Preview.query.get_or_404(id)  # 查找要编辑的标签
    if form.validate_on_submit():
        data = form.data
        # 判断上传文件的文件夹是否存在
        if not os.path.exists(app.config["UP_DIR"]):
...
@admin_login_req
@admin_auth
def user_del(id=None, page=None):  # 删除 跳转到当前页
    user = User.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
...
def comment_del(id=None, page=None):
    comment = Comment.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
...
def moviecol_del(id=None, page=None):
    moviecol = Moviecol.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
...
@admin_login_req
@admin_auth
def role_del(id=None, page=None):  # 删除跳转到当前页
    role = Role.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
...
def role_edit(id=None):
    form = RoleForm()
    role = Role.query.get_or_404(id)  # 查找要编辑的角色
    if request.method == "GET":
        auths = role.auths
...
@admin_login_req
@admin_auth
def auth_del(id=None, page=None):  # 删除跳转到当前页
    auth = Auth.query.filter_by(id=id).first_or_404()  # 查找要删除的标签
